...
files = list.files("~/stoat/bg_points/outputs/models/",
...
# Species list - add column flagging interior west species
...
# write csv
write_csv(mod, "~/stoat/bg_points/outputs/mod_slopes.csv")

###### Trends across slopes (based on annual avgs)
...
t1 = cbind(t1[,1:3],round(t1[4:12]*20,2))
...
for (season in c("breeding","nonbreeding")){
  ...
  for (i in c("lat","elev","dist","tmax","modisevi","prec")){
    ...
    out <- c(season,i,"species_adj", median(mods$dens), 
             sd(mods$dens)/sqrt(nrow(mods)))
    output <- rbind(output, out)
  }
  # What's the distribution of mitigated species niche shifts?
...
output = cbind(output[,1:3],round(as.numeric(output[,4]),2),
                round(as.numeric(output[,5]),2))
...
colnames(output) = c("Variable","Dataset","breeding_Mean","breeding_SE",
                      "nonbreeding_Mean","nonbreeding_SE")
...
# difference between species and baseline
  for (season in c("breeding","nonbreeding")){
...
gga = ggarrange(bl.tmax.breeding, blmit.tmax.breeding, 
                 ...
                 ncol=2, nrow=2)
ggsave("figs/trends/cross_sp_trends_bl_v_sp6.jpeg", 
       gga, "jpeg", height=8, width=10, units="in")
