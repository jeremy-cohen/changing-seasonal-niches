...
# identical between GBIF, ebird, BBS analyses
...
# Import, name species from master list
...
# identify species with contradicting names
...
# load LDI calculated in a different project to the same species seasonally
# Partition seasonally, add to our species level data
...
{# resolve taxonomy, drop all species other than ours
  ...
  # drop any species not included in our dataset, save
...
# IF trait/phylgeny setup was already done previously, load and attach it
...
for (season in c("breeding","nonbreeding")){
  ...
  # Migration only figure - just realized niche shifts -  fig. 4
