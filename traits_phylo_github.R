...
### model relationships between niche and space shift combinations
...
for (i in 1:length(nmetrics)){
  for (j in 1:length(smetrics)){
    svar = spdata[,paste0(smetrics[j],"_coef")][[1]]
    if(smetrics[j]=="dist"){svar=abs(svar)}
    nvar = spdata[,paste0(nmetrics[i],"_sp_corr")][[1]]
    mitvar = spdata[,paste0(nmetrics[i],"_sp_mit")][[1]]
    
    lm = lm(svar ~ nvar + spdata$season)
    summ1 = c(nmetrics[i], smetrics[j], 
               summary(lm)$coef[2,c(1,2,4)],summary(lm)$r.squared)
    
    lm = lm(svar ~ mitvar + spdata$season)
...
for (i in 1:length(responses)){
  ...
  pgls_nocov = gls(formula, correlation=corBrownian(phy=birds.sub, form=~phylo_names),
  ...
  for (pred in predictors){
    ...
    pgls = gls(formula, correlation=corBrownian(phy=birds.sub, form=~phylo_names),
                data=spdata)
    table = round(as.data.frame(summary(pgls)$tTable),3)
...
ggsave("outputs/cross_sp/spatial.jpeg", gga, "jpeg",
       height=9, width=12, units="in")
gga = ggarrange(gg.pr.7.1, gg.pr.7.2, gg.pr.7.3,
                 gg.pr.1.1, gg.pr.1.2, gg.pr.1.3,
                 gg.pr.2.1, gg.pr.2.2, gg.pr.2.3,
                 nrow=3, ncol=3, labels=c("a)","b)","c)","d)",
                                          "e)","f)","g)","h)","i)"))
# hand-wing index only figure
ggsave("outputs/cross_sp/functional_figure3.jpeg", gga, "jpeg",
       height=9, width=10, units="in")
gga = ggarrange(gg.pr.7.4, gg.pr.1.4, gg.pr.2.4,
                 nrow=3, ncol=1, labels=c("a)","b)","c)"))
ggsave("outputs/cross_sp/functional_figure_small2.jpeg", gga, "jpeg",
