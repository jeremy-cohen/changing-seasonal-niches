# code sheet 1
# species-level spatial and niche trend workflow 
# incorporates observer points to adjust for observer bias
# incorporates baseline points to estimate expected exposure and ajust bias
# Code is nearly identical across GBIF/ebird/BBS analyses, with the only exception the loaded datasets
  ...
  # change computing instructions based on local or Yale HPC
  loc <- "hpc" # local or hpc
  if(loc=="local"){
    setwd("~/stoat/bg_points/")
    memory.limit(300000)
  }else{ setwd("/vast/palmer/home.mccleary/jc3893/gbif/bg_points/")
    n.cores <- 5
...
dir.create("outputs/tables/")
 ...
 # load species list, adjust names
  ...
  season = "breeding"
    ...
    # load pre-processed, annotated GBIF data for all species
    alldata <- read_csv(paste0("data/",season,"_processed_20yr.csv"))
    # add month and year
    # push december events to next year so contiguous winter seasons have same year
    alldata$month <- month(alldata$eventdate)
    alldata$year <- ifelse(alldata$month==12 & alldata$year>2009,alldata$year+1,alldata$year)
    # adjust year backwards in winter data so year corresponds to start
...
# loop through species in parallel to find niche and range shifts
    foreach (sp = 1:nrow(list)) %dopar% { 
      library(tidyverse)
      library(rgdal)
      library(sf)
      library(sp)
...
#  for (sp in 1:nrow(list)){
      # cleanup environment
      rm(locdata, spdata); gc()
      # try catch to run more species after one fails
      try({
        # species names and codes based on master list
        sp_name <- list$Scientific[sp]
        sp.name <- gsub("_"," ",sp_name)
        common <- list$Common[sp]
        code <- as.character(list$ebird_code[sp]) 
        rows = nrow(alldata[alldata$sci_name==sp.name,])
        # compile list of species with no data
        if(rows==0){zeroes <- c(zeroes,common)}
        ...
        if(file.exists(paste0("outputs/models/mod_",sp_name,"_",season,".csv"))==F & #CHANGE BACK TO F
          ...
          if(loc=="local"){rangepath<-"~/ranges/range_polygons/"}else{
            rangepath<-"/vast/palmer/home.mccleary/jc3893/30x30/range_polygons/"}
          ...
          # focal species occurrence points from master datasets
          spdata <- alldata[alldata$sci_name==sp.name,] 
          blsp <- bl[bl$species==sp.name,]
          
          # observer (background) dataset creation
          # observer dataset is all species observations within focal species range
          # thin- get unique locations per year for all data in species range
          locdata <- alldata %>% 
          ...
          # restrict summer months in some cases
          # species designated "migrating" were found to have a >2 degree mean
          # difference in their jun-jul vs aug range, so Aug is cut
          # Charadriiformes or shorebirds are early migrants that move in July
          if (season=="breeding" & list$aug_migration[sp]=="migrating"){
            spdata = spdata[spdata$month %in% 6:7,]
            blsp = blsp[blsp$month %in% 6:7,]
            locdata = locdata[locdata$month %in% 6:7,]
          }
          if (season=="breeding" & list$order[sp]=="Charadriiformes"){
            spdata = spdata[spdata$month==6,]
            blsp = blsp[blsp$month==6,]
            locdata = locdata[locdata$month==6,]
          }
          
          # mask focal species points by range to exclude vagrancy
          ...
          # stop species if <500 points
          if(nrow(spdata)<500){STOPLOWDATA}
          
          # mask observer points by range
          ...
          # baseline - check for complete set of years for every location
          # necessary for loop below
          ...
          for (year in 2000:2020){ # year=2018
            ...
            # medians and SEs for each niche dimension per year
            ...
            # spatial centroids and SEs per year
            ...
            # focal species range dimensions
            lim <- st_bbox(rangest)
            ...
            # compile centroids
            centroids <- c(as.numeric(centroids[1,1:6]), as.numeric(centroids[2,1:6]), 
                           as.numeric(centroids[3,1:6]),as.numeric(centroid[1,1:6]), 
                           as.numeric(centroid[2,1:6]), as.numeric(centroid[3,1:6]))
            names(centroids) <- c("tmax.bg","modisevi.bg","prec.bg",
                                  ...
                                  "lon.bg","lat.bg","elev.bg",
                                  "se.lon.bg","se.lat.bg","se.elev.bg",
                                  "lon.sp","lat.sp","elev.sp",
                                  "se.lon.sp","se.lat.sp","se.elev.sp",
                                  "lon.bl","lat.bl","elev.bl",
                                  "se.lon.bl","se.lat.bl","se.elev.bl")
            assign(paste0("c.",year), centroids)
            
          } # end year loop
          
          # save maps with 2000, 2010, 2020 data and centroids
          if(loc=="hpc"){
          ...
          # annual output of centroids, errors for species
          ...
          colnames(out)[1:4] <- c("common_name","scientific_name","season","year")
          # write out
          write_csv(out, paste0("outputs/metrics/out_",sp_name,"_",season,".csv"))
          
          
          # change in 3-dimensional niche dissimilarity between
          # expected and realized niche post-2014
          spdata.late <- spdata[spdata$year>=2014,]
          spdata.late <- spdata.late[complete.cases(spdata.late),]
          spdata.late$prec = round(log(spdata.late$prec+1),5)
          blsp.late <- blsp[blsp$year>=2014,]
          blsp.late <- blsp.late[complete.cases(blsp.late),]
          blsp.late$prec = round(log(blsp.late$prec+1),5)
          # dissimilarity between species pts and baseline pts
          niche_vars <- c("tmax", "modisevi", "prec")
          mvnh.dis <- MVNH_dissimilarity(spdata.late[,niche_vars], 
                                         blsp.late[,niche_vars], metric="Pianka")
          pianka <- mvnh.dis$`Pianka`[1]
          mahal <- mvnh.dis$`Mahalanobis_distance`[1]
          dr <- mvnh.dis$`Determinant_ratio`[1]
          metrics <- round(c(pianka, mahal, dr), 4)
          ...
          ##### Estimation of niche and range shifts over 20yr
          ...
          # baseline only plot
          tmax2.ip <- interact_plot(tmax_lm, pred = year, modx = status, interval=T,
                                   colors=c("transparent","black","transparent"), lty=c(2,2,2)) +
            xlab("Year") +
            ylab("Temperature (°C)") +
            theme_classic() +
            theme(legend.position="none",
                  ...
                  axis.ticks = element_line(linewidth=2)) +
            scale_x_continuous(breaks=c(2000,2010,2020))
          # adjusted focal + baseline plot
          adjslope = em[3,2]-(em[1,2]-em[2,2])
          med = median(spdata$tmax)
          tmax3.ip <- tmax2.ip +
            geom_segment(x=2000, xend=2021, 
                        y=med-(adjslope*10), yend=med+(adjslope*10),
                        col=col, linewidth=2)
          
          # tmax (observer only, as a test)
          ...
          # precipitation (baseline + observer)
          ...
          lat.ip <- interact_plot(lat_lm, pred = year, modx = status, interval=T,
            ...
            scale_x_continuous(breaks=c(2000,2010,2020))
          # adjusted focal + baseline plot
          adjslope = em[2,2]-em[1,2]
          med = median(spbgdata$latitude)
          lat2.ip <- interact_plot(lat_lm, pred = year, modx = status, interval=T,
                                  colors=c("transparent","transparent"), lty=c(2,2)) +
            geom_segment(x=2000, xend=2021, 
                         y=med-(adjslope*10), yend=med+(adjslope*10),
                         col=col, linewidth=2) +
            xlab("Year") +
          ...
          # save plots
          if(loc=="local"){
            gga <- ggarrange(tmax.ip, tmax2.ip, tmax3.ip, modisevi.ip, prec.ip, 
                             lat.ip, lat2.ip, elev.ip, dist.ip, ncol=5, nrow=2)
            ggsave(paste0("figs/trends/",gsub(" ","_",common),"_",
                          season,"_trends_full.jpeg"), gga, "jpeg",
                   height=8, width=20, units="in")
            gga <- ggarrange(lat.ip, lat2.ip, tmax.ip, tmax2.ip, tmax3.ip, ncol=3, nrow=2)
            ggsave(paste0("figs/trends/",gsub(" ","_",common),"_",
            ...
            grid.arrange(tmax.ip, tmax2.ip, tmax3.ip, modisevi.ip, prec.ip, 
                         lat.ip, lat2.ip, elev.ip, dist.ip, ncol=5, nrow=2)
            dev.off()
            Cairo(paste0("figs/trends/",gsub(" ","_",common),"_",
                         season,"_trends_two.jpeg"), type="jpeg",
                  height=800, width=1200)
            grid.arrange(tmax.ip, tmax2.ip, tmax3.ip, 
                         lat.ip, lat2.ip, ncol=3, nrow=2)
            dev.off()
          }
          ...
          # early (2000-2004) vs late (2016-2020) niche plots
          for (type in 1:3){
          ...
          # compile early and late medians
          ...
          model_out <- data.frame(c(common, sp_name, season, metrics,
          ...
          colnames(model_out) <- c("common_name","scientific_name","season","pianka","mahal","dr",
